...
# ẨN TOKEN: Tự động lấy từ Variables của Railway và xóa bỏ ký tự xuống dòng thừa
...
async def process_import_tg(user_dir, zip_path, status_msg, context):
    """Hàm xử lý Import tối ưu dung lượng và bảo vệ cấu trúc nén"""
    ...
    for obj in textures:
        try:
            ...
            # CHỈ MOD KHI NGƯỜI DÙNG THAY ĐỔI CÓ FILE ẢNH
            if os.path.exists(fp):
                pil_img = Image.open(fp).convert("RGBA")
                data.set_image(pil_img, TextureFormat.RGBA32)
                data.save() # Lưu trực tiếp vào object phân mảnh
        except:
            pass
        done += 1
    ...
    # Nén bằng lz4 kết hợp bảo toàn block dữ liệu cũ
...
def main():
    ...
    if not TOKEN:
        print("LỖI CRITICAL: Chưa thiết lập biến môi trường TELEGRAM_TOKEN trên Railway!")
        return
    ...
    print("Bot đang chạy mượt mà trên Server...")
